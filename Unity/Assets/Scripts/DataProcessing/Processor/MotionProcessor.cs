...
public class MotionProcessor : EditorWindow {
	...
	public enum PIPELINE {Quadruped, LaFAN};
	...
	public PIPELINE Pipeline = PIPELINE.LaFAN; 
	...
	private IEnumerator Process() {
		if(Editor != null) {
			...
			if (Pipeline == PIPELINE.LaFAN)
			{	
				Editor.TargetFramerate = 30;
				...
				foreach (Asset a in Assets)
				{
					if (a.Selected)
					{
...

						{
							...
							contact.AddSensor("RightHand", "RightHand", Vector3.zero, 0.15f, 1f, LayerMask.GetMask("Ground"));
							contact.AddSensor("LeftFoot", "LeftFoot", Vector3.zero, 0.15f, 1f, LayerMask.GetMask("Ground"));
							contact.AddSensor("RightFoot", "RightFoot", Vector3.zero, 0.15f, 1f, LayerMask.GetMask("Ground"));
...

 						{
							...
							StyleModule.StyleFunction aiming = style.AddStyle("Aiming");
							StyleModule.StyleFunction crouching = style.AddStyle("Crouching");
							//StyleModule.StyleFunction obstacle = style.AddStyle("Obstacle");
							//jumping
							//obstacles
							...
							for (int f = 0; f < data.Frames.Length; f++)
							{
								...
								float locomotionWeight = weights.Gaussian().SmoothStep(2f, 0.5f);
								float crouchingWeight = GetCrouchingWeight(data.Frames[f].Timestamp);
								float obstacleWeight = GetObstacleWeight(data.Frames[f].Timestamp);
								//idling.Values[f] = Mathf.Clamp(1f - locomotionWeight - crouchingWeight, 0f, 1f);
								moving.Values[f] = Mathf.Clamp(1f - crouchingWeight, 0f, 1f); //Mathf.Clamp(locomotionWeight - crouchingWeight, 0f, 1f);
								aiming.Values[f] = 0f;
								crouching.Values[f] = Mathf.Clamp(crouchingWeight, 0f, 1f);
								//obstacle.Values[f] = Mathf.Clamp(obstacleWeight - crouchingWeight, 0f, 1f);
								//speed.Values[f] = GetRootSpeed(data.Frames[f].Timestamp);
							}

							float GetCrouchingWeight(float timestamp)
							{
								return Compute();
								float Compute()
								{
									float[] contacts = new float[timeWindow.Length];
									for (int i = 0; i < timeWindow.Length; i++)
									{
										contacts[i] = IsCrouching(timestamp + timeWindow[i]) ? 1f : 0f;
										contacts[i] = contacts[i].Normalize(0f, threshold, 0, 1f);
										contacts[i] = contacts[i].SmoothStep(2f, 0.5f);
									}
									float w = contacts.Gaussian().SmoothStep(2f, 0.5f);
									return w;
								} 
							}
							bool IsCrouching(float timestamp)
							{
								return Compute();
								bool Compute()
								{
									...
									ContactModule.Sensor hip = contact.GetSensor("Hips");
									ContactModule.Sensor leftHand = contact.GetSensor("LeftHand");
									ContactModule.Sensor rightHand = contact.GetSensor("RightHand");
									ContactModule.Sensor leftFoot = contact.GetSensor("LeftFoot");
									ContactModule.Sensor rightFoot = contact.GetSensor("RightFoot");
									if(leftHand == null || rightHand == null || leftFoot == null || rightFoot == null) {
										return false;
									}
									return (hip.GetContact(timestamp, false) == 1f) ||
										   (leftHand.GetContact(timestamp, false) == 1f || rightHand.GetContact(timestamp, false) == 1f);
								}
							}

							float GetObstacleWeight(float timestamp)
							{
								return Compute();
								float Compute()
								{
									...
									for (int i = 0; i < timeWindow.Length; i++)
									{
										contacts[i] = OnObstacle(timestamp + timeWindow[i]) ? 1f : 0f;
										contacts[i] = contacts[i].Normalize(0f, threshold, 0, 1f);
										contacts[i] = contacts[i].SmoothStep(2f, 0.5f);
									}
									float w = contacts.Gaussian().SmoothStep(2f, 0.5f);
							...
							bool OnObstacle(float timestamp)
							{
								return Compute();
								bool Compute()
								{
									...
									Frame f = data.GetFrame(timestamp);
									return (hip.GetContact(timestamp, false) == 0f && leftHand.GetContact(timestamp, false) == 0f && rightHand.GetContact(timestamp, false) == 0f
											&& leftFoot.GetContact(timestamp, false) == 0f && rightFoot.GetContact(timestamp, false) == 0f) &&
											f.GetBoneTransformation("Hips", false).GetPosition().y > 1.02f;
								}
							}
						...
						
						{
							...
							//Import Deep Phases
 							string path = "Assets/Demo/Authoring/MotionCapture/Biped/DeepPhases/LaFAN1_Full";
							string[] folders = AssetDatabase.GetSubFolders(path);
							for(int i=0; i<folders.Length; i++)
							{
								string filter = Path.GetFileName(folders[i]).Split('.')[0];
								if(!filter.Equals(data.GetName())) {
									continue;
								} else {
									//found match
									string filePathMirrored = folders[i] + "/Phases_Mirrored.txt";
									string filePathRegular = folders[i] + "/Phases_Standard.txt";
									betweening.MirroredTextAsset = (TextAsset)AssetDatabase.LoadMainAssetAtPath(filePathMirrored);
									betweening.RegularTextAsset = (TextAsset)AssetDatabase.LoadMainAssetAtPath(filePathRegular);
									betweening.ImportPhases();
									break;
								}
							}  
						}
						
						{
							...
							phase.ShowWindow = true;
							phase.DisplayValues = true;
							phase.MaxIterations = 10;
...
/* 					asset.ResetSequences();
					//Editor.GetAsset(i).Export = false;
					asset.SetSequence(0, 120, asset.GetFrame(asset.Frames.Length - 1).Index); */
					asset.Export = true;
				}
				...
				foreach(Asset a in Assets) {
					if(a.Selected) {
...

						{
							...
							contact.AddSensor("LeftHandSite", "LeftHandSite", Vector3.zero, 1f/30f, 1f, LayerMask.GetMask("Ground"));
							contact.AddSensor("RightHandSite", "RightHandSite", Vector3.zero, 1f/30f, 1f, LayerMask.GetMask("Ground"));
							contact.AddSensor("LeftFootSite", "LeftFootSite",  Vector3.zero, 1f/30f, 1f, LayerMask.GetMask("Ground"));
							contact.AddSensor("RightFootSite", "RightFootSite",  Vector3.zero, 1f/30f, 1f, LayerMask.GetMask("Ground"));
							contact.CaptureContacts(Editor);
						}
...
/* 						{
						...
						{
							InBetweeningModule betweening = data.AddModule<InBetweeningModule>();
							betweening.ChannelCount = 5;
						}
						{
							PhaseModule phase = data.AddModule<PhaseModule>();
