...
public class AnimatorImporter : MonoBehaviour {
	...
	private void PostProcess() {
		...
		for(int i=0; i<Retarget.Bones.Length; i++) {
			Retarget.Bones[Mapping[i]].GetTransform().position = posture[i].GetPosition();
			Retarget.Bones[Mapping[i]].GetTransform().rotation = posture[i].GetRotation();
		}
	}
						...
						if(!Directory.Exists(destination+"/"+name) ) {
							...
							if(!Demo) {
								...
								for(int i=0; i<frames.Count; i++) {
									data.Frames[i] = new Frame(data, i+1, frames[i].Timestamp, frames[i].WorldPosture);
								}
	...
	public void ComputeDelta() {
		if(Retarget != null & Skeleton != null) {
			if(!Application.isPlaying && transform.hasChanged) {
				...
				for(int i=0; i<Retarget.Bones.Length; i++) {
					Delta[i] = Retarget.Bones[Mapping[i]].GetTransform().rotation.GetRelativeRotationTo(Skeleton.Bones[i].GetTransform().GetWorldMatrix()).eulerAngles;
				}
			}
		}
	}
	...
	public class AnimatorImporter_Editor : Editor {
		...
		public override void OnInspectorGUI() {
			...
			if(!Target.Baking) {
				...
				EditorGUI.DrawRect(new Rect(EditorGUILayout.GetControlRect().x, EditorGUILayout.GetControlRect().y, Target.GetNormalizedTimestamp() * EditorGUILayout.GetControlRect().width, 25f), UltiDraw.Cyan.Opacity(0.75f));
