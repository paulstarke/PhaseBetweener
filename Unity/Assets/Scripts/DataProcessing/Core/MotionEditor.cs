...
public class MotionEditor : MonoBehaviour {
	...
	public void ApplyCameraFocus() {
		...
		if(CameraFocus) {
			if(SceneView.lastActiveSceneView != null) {
				...
				Vector3 target = (GetActor().Bones.Length == 0 ? GetActor().GetRoot().position : GetActor().Bones.First().GetTransform().position).SetY(FocusHeight);
	...
	public void ApplyMotionTrail() {
		...
		if(MotionTrail) {
			...
			int time = 2; //(int)Data.GetModule<InBetweeningModule>().FuturePose.TimeOffset;//SampleFuturePose(Timestamp, Mirror, GetActor().GetBoneNames(), 1, 60).TimeOffset;
			int num = 10;
			int step = (int)(time*TargetFramerate)/num;
			int keys = 4;
			for(int i=0; i<keys*time*TargetFramerate; i+= step) {
				...
				if(instance == null) {
					...
					transparency.SetTransparency(0.5f);//(i == series.Pivot ? 0f : 1f - (float)Mathf.Abs(i-series.Pivot) / (float)series.Pivot);
					instance.SetParent(trail);
				}
				Actor actor = instance.GetComponent<Actor>();
				actor.DrawSkeleton = SkeletonSketchTrail;
				if((i+step) % (time*TargetFramerate) == 0){
					actor.BoneColor = UltiDraw.Green;
					actor.DrawSkeleton = true;
					instance.gameObject.GetComponent<Transparency>().SetTransparency(1.0f);
				} else {
					actor.BoneColor = UltiDraw.Cyan;//.Darken((float)Mathf.Abs(i- GetTimeSeries().Pivot) / (float) GetTimeSeries().Pivot);
				...
				for(int b=0; b<actor.Bones.Length; b++) {
					Matrix4x4 bone = Data.GetFrame(GetCurrentFrame().Timestamp +  (i+step)/TargetFramerate).GetBoneTransformation(actor.Bones[b].GetName(), Mirror);
	...
	public void LoadFrame(float timestamp) {
		...
		for(int i=0; i<GetActor().Bones.Length; i++) {
			if(GetBoneMapping()[i] == -1) {
				...
				GetActor().Bones[i].GetTransform().position = transformation.GetPosition();
				GetActor().Bones[i].GetTransform().rotation = transformation.GetRotation();
				GetActor().Bones[i].SetVelocity(velocity);
			}
		}
		#if UNITY_EDITOR
		//Apply scene changes.
	...
	void OnRenderObject() {
		if(Data != null && Visualize) {
			...
			if(VisualizeFirstFrame) {
				GetActor().Draw(Data.GetFirstValidFrame().GetBoneTransformations(BoneMapping, Mirror), Color.black, GetActor().JointColor, Actor.DRAW.Skeleton);
			}
			if(VisualizeLastFrame) {
				GetActor().Draw(Data.GetLastValidFrame().GetBoneTransformations(BoneMapping, Mirror), Color.black, GetActor().JointColor, Actor.DRAW.Skeleton);
			}
		}
	}
