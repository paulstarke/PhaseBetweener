...
public abstract class Module : ScriptableObject {
	...
	public enum ID {Root, Style, Phase, Contact, HeightMap, DepthMap, CylinderMap, CuboidMap, SphereMap, DistanceMap, Motion, Alignment, Dribble, InBetweening, Length};
	...
	public void GUI(MotionEditor editor) {
		if(Visualize) {
			ComponentSeries series = ExtractSeries(editor.GetTimeSeries(), editor.GetCurrentFrame().Timestamp, editor.Mirror);
			if(series != null) {
				series.GUI();
			}
			DerivedGUI(editor);
