...
public class FootSliding : MonoBehaviour {
	...
	public NeuralONNXAnimation[] Animations = new NeuralONNXAnimation[0];
	...
	void LateUpdate () {
		 for(int i=0; i<Animations.Length; i++) {
		     //Compute Value
		 	Matrix4x4 left = Animations[i].Actor.GetBoneTransformation("LeftFoot");
		 	Matrix4x4 right = Animations[i].Actor.GetBoneTransformation("RightFoot");

		 	ContactSeries contact =  (ContactSeries)Animations[i].GetComponent<AuthoringInBetweeningController>().GetContactSeries(); //ToDo: Left & Right Foot extract
			
		 	float value =
		 		contact.Values[contact.Pivot][3] * Vector3.Distance(PreviousTransformations[i][0].GetPosition(), left.GetPosition())
		 		+ contact.Values[contact.Pivot][4] * Vector3.Distance(PreviousTransformations[i][1].GetPosition(), right.GetPosition());
...
/* 		 	while(Values[i].Count > Frames) {
		...
		for(int i=0; i<Editors.Length; i++) {
			...
			Matrix4x4 left = frame.GetBoneTransformation("LeftFoot", Editors[i].Mirror);
			Matrix4x4 right = frame.GetBoneTransformation("RightFoot", Editors[i].Mirror);
			ContactModule.Sensor[] sensors = (Editors[i].GetAsset().GetModule<ContactModule>()).GetSensors("LeftFoot", "RightFoot");
			float value =
...
/* 			while(Values[i].Count > Frames) {
	...
	void OnGUI() {
		...
		for(int i=0; i<Animations.Length; i++) {
			...
			UltiDraw.OnGUILabel(new Vector2(Rect.X - 0.5f * Rect.W, Rect.Y - 0.5f*Rect.H - (i+1)*size), Rect.GetSize(), size/2f, mean.Round(3).ToString(), UltiDraw.Red);
			UltiDraw.OnGUILabel(new Vector2(Rect.X, Rect.Y - 0.5f*Rect.H - (i+1)*size), Rect.GetSize(), size/2f, Animations[i].name, UltiDraw.Black);
			UltiDraw.OnGUILabel(new Vector2(Rect.X + 0.5f * Rect.W, Rect.Y - 0.5f*Rect.H - (i+1)*size), Rect.GetSize(), size/2f, sigma.Round(3).ToString(), UltiDraw.Red);
		}
		for(int i=0; i<Editors.Length; i++) {
			float mean = values[i].Mean();
			float sigma = values[i].Sigma();
