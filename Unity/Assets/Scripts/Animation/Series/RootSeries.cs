...
public class RootSeries : ComponentSeries {
    ...
    public RootSeries(TimeSeries global) : base(global) {
        ...
        Lengths = new float[Samples.Length];
        Arcs = new float[Samples.Length];
        DeltaTimeOffsets = new float[Samples.Length];
        for(int i=0; i<Samples.Length; i++) {
            Transformations[i] = Matrix4x4.identity;
            Velocities[i] = Vector3.zero;
            Lengths[i] = 0f;
            Arcs[i] = 0f;
            DeltaTimeOffsets[i] = 0f;
        }
    }
    ...
    public RootSeries(TimeSeries global, Transform transform) : base(global) {
        ...
        Arcs = new float[Samples.Length];
        DeltaTimeOffsets = new float[Samples.Length];
        Matrix4x4 root = transform.GetWorldMatrix();
        for(int i=0; i<Samples.Length; i++) {
    ...
    public void SetArc(int index, float value) {
        Arcs[index] = value;
    }

    public float GetArc(int index) {
        return Arcs[index];
    }
...
/*         UltiDraw.Begin(canvas);
        ...
        int step = Resolution;
        //DeltaTimeOffsets
        for(int i=0; i<DeltaTimeOffsets.Length; i+=step) {
            Debug.Log(DeltaTimeOffsets[i]);
            GUIStyle style = new GUIStyle();
            style.fontSize = (int)HandleUtility.GetHandleSize(Transformations[i].GetPosition());
            Handles.Label(Transformations[i].GetPosition(), DeltaTimeOffsets[i] + "s", style);
            UltiDraw.OnGUILabel(Transformations[i].GetPosition().ToXZ() + new Vector2(0f, 0.03f), new Vector2(0.1f, 0.1f), 0.0175f, DeltaTimeOffsets[i] + "s", UltiDraw.Black);
        }
        UltiDraw.End(); */
    }
    ...
    public override void Draw(Camera canvas=null) {
        if(DrawScene) {
            ...
            //Lengths
            for(int i=0; i<Lengths.Length; i+=step) {
                UltiDraw.DrawLine(Transformations[i].GetPosition(), Transformations[i].GetPosition() + GetTemporalScale(Lengths[i]) * GetDirection(i), Transformations[i].GetUp(), size*0.0125f, 0f, UltiDraw.DarkRed.Opacity(0.5f));
            }

            //Arcs
            for(int i=0; i<Arcs.Length; i+=step) {
                UltiDraw.DrawLine(Transformations[i].GetPosition(), Transformations[i].GetPosition() + GetTemporalScale(Arcs[i]) * Transformations[Pivot].GetRight(), 0.05f, 0f, UltiDraw.Blue.Opacity(0.5f));
            }
