...
public class DribbleSeries : ComponentSeries {
    ...
    public DribbleSeries(TimeSeries global, float area, Transform ball, Actor actor, RootSeries actorRoot, Actor rival, RootSeries rivalRoot) : base(global) {
        ...
        for(int i=0; i<Samples.Length; i++) {
            BallTransformations[i] = ball.transform.GetWorldMatrix();
    ...
    public float GetInteractorBoneDistance(int index) {
        ...
        return Mathf.Clamp(Vector3.Distance(Actor.Bones[index].GetTransform().position, Rival.Bones[index].GetTransform().position), 0f, GetInteractionRadius());
    }
        ...
        if(DrawScene) {
            if(Rival != null && RivalRoot != null) {
                ...
                }
                {
                    ...
                    for(int i=0; i<Actor.Bones.Length; i++) {
                        UltiDraw.DrawLine(Actor.Bones[i].GetTransform().position, Rival.Bones[i].GetTransform().position, UltiDraw.Red.Opacity(0.25f*weight), UltiDraw.Black.Opacity(0.25f*weight));
                    }
                }
            }
        }
